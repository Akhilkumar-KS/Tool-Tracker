...
#Load YOLO Algorithm
net=cv2.dnn.readNet("additional\yolov4-obj_best.weights","additional\yolov4-obj.cfg")
...
def detect(path):
    ...
    for output in outs:
        for detection in output:
            ...
            if confidence >0.005: #Means if the object is detected
    ...
    indexes=cv2.dnn.NMSBoxes(boxes,confidences,0.001,0.001)
    print(class_ids)
    print(indexes,confidences,boxes)

    classList={}
    for i in indexes:
        key=classes[class_ids[i]]
        if key in classList:
            classList[key]+=classList[key]
        else:
            classList[key]=1
